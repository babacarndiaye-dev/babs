    ...
    <div class="grid lg:grid-cols-3 gap-6">
                ...
                <tbody class="divide-y divide-black/5">
                    @forelse ($subjects as $subject)
                        <tr wire:key="subject-{{ $subject->id }}">
                            <td class="px-5 py-3.5 font-medium text-ink">{{ $subject->name }}</td>
                            <td class="px-5 py-3.5 text-ink/60">{{ $subject->code }}</td>
                            <td class="px-5 py-3.5 text-right space-x-3">
                                <button wire:click="edit({{ $subject->id }})" class="text-primary hover:underline font-medium">Modifier</button>
                                <button wire:click="delete({{ $subject->id }})" wire:confirm="Supprimer cette matière ?"
                                        class="text-red-600 hover:underline font-medium">Supprimer</button>
                            </td>
                        </tr>
                    @empty
                        <tr><td colspan="3" class="px-5 py-12 text-center text-ink/50">Aucune matière.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
        ...
        <div>
            <form wire:submit="save" class="rounded-2xl border border-black/5 bg-white p-6 space-y-4">
                <h3 class="font-semibold text-ink">{{ $editing ? 'Modifier la matière' : 'Nouvelle matière' }}</h3>
                <div>
